...
// What the page may load and talk to: itself plus the API (fetch and WebSocket).
...
const csp = [
  ...
  "font-src 'self' data:",
  "media-src 'self' blob:",
  `connect-src 'self' ${originOf(apiUrl)} ${originOf(wsUrl)}`.trim(),
...
const securityHeaders = [
  ...
  // This app uses the microphone (voice mode) and nothing else.
