"""Google Gemini. See
https://docs.langchain.com/oss/python/langchain/models#google-gemini."""

from __future__ import annotations

import os

from langchain_google_genai import ChatGoogleGenerativeAI

from .base import LlmProviderBase, ProviderConfig
from .registry import register

_DEFAULT_MODEL = "gemini-3.7-flash"


@register
class GeminiProvider(LlmProviderBase):
    name = "gemini"
    priority = 30

    def detect(self, env) -> bool:
        return bool(env.get("GOOGLE_API_KEY"))

    def resolve(self, model_override: str | None, env) -> ProviderConfig:
        api_key = env["GOOGLE_API_KEY"]
        model = model_override or env.get("GEMINI_MODEL", _DEFAULT_MODEL)
        return ProviderConfig(name=self.name, model=model, api_key=api_key)

    def build_model(self, provider: ProviderConfig, **model_kwargs):
        # ChatGoogleGenerativeAI doesn't accept stream_usage -- passing it
        # through emits a UserWarning and silently stuffs it into
        # model_kwargs instead of doing anything useful.
        model_kwargs.pop("stream_usage", None)
        return ChatGoogleGenerativeAI(model=provider.model, api_key=provider.api_key, **model_kwargs)

    def price_for(self, provider: ProviderConfig) -> tuple[float, float] | None:
        env_in = os.environ.get("GEMINI_PRICE_INPUT_PER_1K")
        env_out = os.environ.get("GEMINI_PRICE_OUTPUT_PER_1K")
        if env_in is not None and env_out is not None:
            return (float(env_in), float(env_out))
        return None

    def context_window_for(self, provider: ProviderConfig) -> int | None:
        env_val = os.environ.get("GEMINI_CONTEXT_WINDOW")
        if env_val is not None:
            return int(env_val)
        return None
